...
func TestMasjidBoardHierarchyReturnsPersistedHierarchy(t *testing.T) {
	...
	if err := hierarchy.NewStore(path).Save(state); err != nil {
		t.Fatal(err)
	}
	...
	if len(got.Countries) != 1 || got.Countries[0].Regions[0].Cities[0].Name != "Brits" {
		t.Fatalf("got=%+v", got)
	}
}
...
func TestMasjidBoardScopeRoundTripMultipleLocations(t *testing.T) {
	...
	if !response.Configured || len(response.Locations) != 2 || response.Locations[1].City != "Akasia" {
