...
// Source retrieves a validated provider-neutral catalogue candidate for one
// configured location. Each location is refreshed independently.
type Source interface {
	Fetch(context.Context, Location, time.Time) (Catalogue, error)
}

// PartitionPersistence is the disk-first last-known-good store used by the
// updater. SavePartition must replace only the supplied location partition.
type PartitionPersistence interface {
	Load() (State, error)
	SavePartition(Partition) error
}

// Updater refreshes the configured 1-3 discovery locations independently.
// A failed location retains its previous persisted partition while successful
// locations may still advance.
type Updater struct {
	Source          Source
...
type RefreshResult struct {
	Locations []LocationRefreshResult
}
...
// RefreshScheduled refreshes only location partitions that are missing or at
// least RefreshInterval old. Fresh partitions cause no upstream request.
...
// RefreshManual attempts every configured location immediately, regardless of
// persisted partition age.
...
func (u Updater) refresh(ctx context.Context, locations []Location, now time.Time, force bool) (RefreshResult, error) {
	...
	for _, location := range locations {
		...
		partition := Partition{
			Location:    location,
			RetrievedAt: reconciled.RetrievedAt,
			ValidatedAt: reconciled.ValidatedAt,
