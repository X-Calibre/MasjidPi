...
func TestCatalogueUpdaterScheduledRefreshesOnlyDueLocations(t *testing.T) {
	...
	store := &fakePartitionStore{state: State{Partitions: []Partition{{
		Location: brits, RetrievedAt: now.Add(-24 * time.Hour), ValidatedAt: now.Add(-24 * time.Hour),
		Records: candidate(brits, "masjidboardlive:brits-jamia", "Brits Jamia", now.Add(-24*time.Hour)).Records,
	}}}}
	source := &fakeCatalogueSource{results: map[string]Catalogue{
		fawkner.key(): candidate(fawkner, "masjidboardlive:fawkner-masjid", "Fawkner Masjid", now),
	}, errors: map[string]error{}}
...
func TestCatalogueUpdaterFailureIsolation(t *testing.T) {
	...
	source := &fakeCatalogueSource{results: map[string]Catalogue{
		a.key(): candidate(a, "masjidboardlive:brits-jamia", "Brits Jamia", now),
	}, errors: map[string]error{b.key(): errors.New("offline")}}
