...
func (f *fakeCatalogueSource) Fetch(_ context.Context, location Location, _ time.Time) (Catalogue, error) {
	...
	if err := f.errors[key]; err != nil { return Catalogue{}, err }
...
func (s *fakePartitionStore) SavePartition(partition Partition) error {
	...
	for i := range s.state.Partitions {
		if s.state.Partitions[i].Location.key() == key { s.state.Partitions[i] = partition; return nil }
	}
...
func (s *fakePartitionStore) RetainLocations(locations []Location) error {
	...
	for _, location := range locations { allowed[location.key()] = struct{}{} }
	kept := s.state.Partitions[:0]
	for _, partition := range s.state.Partitions {
		if _, ok := allowed[partition.Location.key()]; ok { kept = append(kept, partition) }
	}
...
func TestCatalogueUpdaterScheduledRefreshesOnlyDueLocations(t *testing.T) {
	...
	if len(source.calls) != 1 || source.calls[0] != fawkner.key() { t.Fatalf("calls=%v", source.calls) }
	if result.Locations[0].Attempted || !result.Locations[1].Attempted || !result.Locations[1].Updated { t.Fatalf("result=%+v", result) }
}
...
func TestCatalogueUpdaterFailureIsolation(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if !result.AnyFailed() || len(store.saves) != 1 || store.saves[0].Location.key() != a.key() { t.Fatalf("result=%+v saves=%+v", result, store.saves) }
	if store.state.Partitions[0].Location.key() == b.key() && store.state.Partitions[0].Records[0].Name != "Old B" { t.Fatal("failed location lost last-known-good partition") }
}
...
func TestCatalogueUpdaterReconcilesMissingPerLocation(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if len(store.saves) != 1 || store.saves[0].Records[0].Status != StatusMissing { t.Fatalf("saved=%+v", store.saves) }
}
...
func TestCatalogueUpdaterPrunesRemovedLocations(t *testing.T) {
	...
	if _, err := (Updater{Source: source, Store: store}).RefreshScheduled(context.Background(), []Location{keep}, now); err != nil { t.Fatal(err) }
	if len(store.state.Partitions) != 1 || store.state.Partitions[0].Location.key() != keep.key() { t.Fatalf("partitions=%+v", store.state.Partitions) }
}
...
func TestCatalogueUpdaterRejectsInvalidLocationSet(t *testing.T) {
	...
	if _, err := updater.RefreshManual(context.Background(), nil, time.Now()); err == nil { t.Fatal("expected empty location error") }
	loc := Location{Country: "South Africa", City: "Brits"}
	if _, err := updater.RefreshManual(context.Background(), []Location{loc, loc}, time.Now()); err == nil { t.Fatal("expected duplicate location error") }
}
