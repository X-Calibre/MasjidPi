...
func (c DiscoveryClient) Cities(ctx context.Context, country, region string) ([]HierarchyEntry, error) {
	...
	entries, err := parseHierarchyPairs(top[0], true)
	if err != nil {
...
func parseHierarchyPairs(raw []byte, allowBlankName bool) ([]HierarchyEntry, error) {
	...
	for i, row := range rows {
		...
		if name == "" && !allowBlankName {
			return nil, fmt.Errorf("row %d name is blank", i)
		}
		count, err := strconv.Atoi(strings.TrimSpace(countText))
		if err != nil || count < 0 {
			return nil, fmt.Errorf("row %d invalid count %q", i, countText)
		}
		entries = append(entries, HierarchyEntry{Name: name, Count: count})
	}
	return entries, nil
}
