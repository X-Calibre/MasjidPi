...
func New(config Config) (*Service, error) {
	...
	if err != nil { return nil, fmt.Errorf("masjidboard service: load selection: %w", err) }
...
func buildCoordinator(state selection.State, cacheStore runtime.CacheStore, factory providerFactory) (*runtime.Coordinator, error) {
	if !state.Configured() { return runtime.New(nil, cacheStore) }
	if err := selection.Validate(state); err != nil { return nil, fmt.Errorf("masjidboard service: invalid selection: %w", err) }
	if cacheStore == nil { return nil, fmt.Errorf("masjidboard service: cache store is required") }
	if factory == nil { return nil, fmt.Errorf("masjidboard service: provider factory is required") }
	items := make([]runtime.Item, 0, len(state.Boards))
	for i, board := range state.Boards {
		p, err := factory(board)
		if err != nil { return nil, fmt.Errorf("masjidboard service: construct provider %d: %w", i+1, err) }
		items = append(items, runtime.Item{Selection: board, Provider: p})
	}
	coordinator, err := runtime.New(items, cacheStore)
	if err != nil { return nil, fmt.Errorf("masjidboard service: create runtime: %w", err) }
	return coordinator, nil
}

func (s *Service) Reconfigure(state selection.State) error {
	if s == nil { return fmt.Errorf("masjidboard service: service is unavailable") }
	if err := selection.Validate(state); err != nil { return err }
	s.mu.RLock(); cacheStore, factory, selectionStore := s.cacheStore, s.factory, s.selectionStore; s.mu.RUnlock()
	coordinator, err := buildCoordinator(state, cacheStore, factory)
	if err != nil { return err }
	if selectionStore == nil { return fmt.Errorf("masjidboard service: selection store is unavailable") }
	if err := selectionStore.Save(state); err != nil { return fmt.Errorf("masjidboard service: persist selection: %w", err) }
	s.mu.Lock(); s.selection, s.runtime, s.results = cloneSelection(state), coordinator, nil; s.mu.Unlock()
	return nil
}
...
func (s *Service) updateDisplayPreference(update func(*selection.State), label string) error {
	if s == nil { return fmt.Errorf("masjidboard service: service is unavailable") }
	s.mu.RLock(); state, store := cloneSelection(s.selection), s.selectionStore; s.mu.RUnlock()
	if !state.Configured() { return fmt.Errorf("masjidboard service: select at least one board before choosing a display %s", label) }
	if store == nil { return fmt.Errorf("masjidboard service: selection store is unavailable") }
	update(&state)
	if err := selection.Validate(state); err != nil { return err }
	if err := store.Save(state); err != nil { return fmt.Errorf("masjidboard service: persist %s: %w", label, err) }
	s.mu.Lock(); s.selection.Layout, s.selection.Theme = state.Layout, state.Theme; s.mu.Unlock()
	return nil
}

func (s *Service) Configured() bool {
	if s == nil { return false }
	s.mu.RLock(); defer s.mu.RUnlock(); return s.selection.Configured()
}

func (s *Service) Selection() selection.State {
	if s == nil { return selection.State{} }
	s.mu.RLock(); defer s.mu.RUnlock(); return cloneSelection(s.selection)
}

func (s *Service) Refresh(ctx context.Context) []runtime.Result {
	if s == nil { return nil }
	s.mu.RLock(); coordinator := s.runtime; s.mu.RUnlock()
	if coordinator == nil { return nil }
	results := coordinator.FetchAll(ctx)
	s.mu.Lock(); if s.runtime == coordinator { s.results = append([]runtime.Result(nil), results...) }; s.mu.Unlock()
	return append([]runtime.Result(nil), results...)
}

func (s *Service) Results() []runtime.Result {
	if s == nil { return nil }
	s.mu.RLock(); defer s.mu.RUnlock(); return append([]runtime.Result(nil), s.results...)
}
