...
func New(player Player, cfg Config) *Manager {
	if cfg.RetryInterval == 0 { cfg.RetryInterval = DefaultRetryInterval }
...
func (m *Manager) Play(selected stream.Stream) {
	...
	if persistence != nil { if err := persistence.Save(selected.ID); err != nil { m.logPersistenceError("saving last playback stream", err) } }
	m.notify()
}

func (m *Manager) Stop() {
	m.mu.Lock(); m.listening = false; m.state = StateIdle; m.lastError = ""; persistence := m.persistence; m.updateStatusLocked(nil); m.mu.Unlock()
	if persistence != nil { if err := persistence.Clear(); err != nil { m.logPersistenceError("clearing last playback stream", err) } }
	m.notify()
}
...
func (m *Manager) run(ctx context.Context) {
	loop := time.NewTicker(stateLoopInterval); defer loop.Stop()
	statusTicker := time.NewTicker(m.statusInterval); defer statusTicker.Stop()
	m.mu.Lock(); availability := m.availability; m.mu.Unlock()
	var availabilityEvents <-chan string
	if availability != nil { availabilityEvents = availability.Events() }
	active := false; activeURL := ""; playing := false; attemptStarted := time.Time{}; nextAttempt := time.Time{}; retryAttempt := 0; reconnectAttempt := 0
	for {
		select {
		case <-ctx.Done(): if active { _ = m.player.Stop() }; return
		case <-m.wake: m.step(ctx, &active, &activeURL, &playing, &attemptStarted, &nextAttempt, &retryAttempt, &reconnectAttempt)
		case <-availabilityEvents: m.step(ctx, &active, &activeURL, &playing, &attemptStarted, &nextAttempt, &retryAttempt, &reconnectAttempt)
		case <-loop.C: m.step(ctx, &active, &activeURL, &playing, &attemptStarted, &nextAttempt, &retryAttempt, &reconnectAttempt)
		case <-statusTicker.C: if active { m.checkPlayerStatus(&active, &activeURL, &playing, &attemptStarted, &nextAttempt, &retryAttempt, &reconnectAttempt) }
		}
	}
}
...
func (m *Manager) step(ctx context.Context, active *bool, activeURL *string, playing *bool, attemptStarted *time.Time, nextAttempt *time.Time, retryAttempt, reconnectAttempt *int) {
	selected, listening, availability := m.snapshot(); if ctx.Err() != nil { return }
	if !listening || selected == nil {
		if *active { _ = m.player.Stop(); *active = false; *activeURL = ""; *playing = false }
		*attemptStarted = time.Time{}; *nextAttempt = time.Time{}; *retryAttempt = 0; *reconnectAttempt = 0; m.setState(StateIdle, "", nil); return
	}
	if availability != nil {
		available, known := availability.IsAvailable(selected.ID)
		if !known || !available {
			if *active { _ = m.player.Stop(); *active = false; *activeURL = ""; *playing = false }
			*attemptStarted = time.Time{}; *nextAttempt = time.Time{}; *retryAttempt = 0; *reconnectAttempt = 0; m.setState(StateWaiting, "", nil); return
		}
	}
	if *active && *activeURL != selected.URL {
		_ = m.player.Stop(); *active = false; *activeURL = ""; *playing = false; *attemptStarted = time.Time{}; *nextAttempt = time.Time{}; *retryAttempt = 0; *reconnectAttempt = 0
	}
	if *active { return }
	if !nextAttempt.IsZero() && time.Now().Before(*nextAttempt) { m.setState(StateRetrying, m.retryMessage(*nextAttempt), nil); return }
	m.setState(StateConnecting, "", nil)
	if err := m.player.Play(selected.URL); err != nil {
		delay := backoffDelay(m.retryInterval, *retryAttempt); (*retryAttempt)++; *nextAttempt = time.Now().Add(delay); *attemptStarted = time.Time{}; m.setState(StateRetrying, err.Error(), nil); m.logRetry(selected, "relay connection failed", err, delay); return
	}
	*active = true; *activeURL = selected.URL; *playing = false; *attemptStarted = time.Now(); *nextAttempt = time.Time{}; *reconnectAttempt = 0; m.setState(StateConnecting, "", nil)
}
...
func (m *Manager) checkPlayerStatus(active *bool, activeURL *string, playing *bool, attemptStarted *time.Time, nextAttempt *time.Time, retryAttempt, reconnectAttempt *int) {
	...
	status, err := m.player.Status()
	if err != nil {
		delay := backoffDelay(m.retryInterval, *retryAttempt); (*retryAttempt)++; _ = m.player.Stop(); *active = false; *activeURL = ""; *playing = false; *attemptStarted = time.Time{}; *nextAttempt = time.Now().Add(delay); m.setState(StateRetrying, err.Error(), nil); m.logRetryFromStatus("player status failed", err, delay); return
	}
	selected, listening, availability := m.snapshot(); if !listening || selected == nil { return }
	if availability != nil {
		available, known := availability.IsAvailable(selected.ID)
		if !known || !available {
	...
	*playing = true; *attemptStarted = time.Time{}; *retryAttempt = 0; *reconnectAttempt = 0
	if err := m.restoreVolume(&status.Volume); err != nil {
		delay := backoffDelay(m.retryInterval, *retryAttempt); (*retryAttempt)++; _ = m.player.Stop(); *active = false; *activeURL = ""; *playing = false; *attemptStarted = time.Time{}; *nextAttempt = time.Now().Add(delay); m.setState(StateRetrying, err.Error(), status); m.logRetryFromStatus("restoring volume after player recovery", err, delay); return
	}
...
func backoffDelay(base time.Duration, attempt int) time.Duration {
	if base <= 0 { base = DefaultRetryInterval }; if attempt < 0 { attempt = 0 }; delay := base
	for i := 0; i < attempt && delay < maxRetryDelay; i++ { if delay > maxRetryDelay/2 { return maxRetryDelay }; delay *= 2 }
	if delay > maxRetryDelay { return maxRetryDelay }; return delay
}
func (m *Manager) retryMessage(nextAttempt time.Time) string { remaining := time.Until(nextAttempt).Round(time.Second); if remaining < 0 { remaining = 0 }; return "Retrying in " + remaining.String() }
func (m *Manager) logRetry(selected *stream.Stream, reason string, err error, delay time.Duration) { if m.log == nil { return }; args := []any{"stream_id", selected.ID, "stream_name", selected.Name, "retry_in", delay.String(), "reason", reason}; if err != nil { args = append(args, "error", err) }; m.log.Warn("Stream playback retry scheduled", args...) }
func (m *Manager) logRetryFromStatus(reason string, err error, delay time.Duration) { if m.log == nil { return }; args := []any{"retry_in", delay.String(), "reason", reason}; if err != nil { args = append(args, "error", err) }; m.log.Warn("Stream playback retry scheduled", args...) }
func (m *Manager) logPersistenceError(action string, err error) { if m.log != nil { m.log.Warn("Playback persistence failed", "action", action, "error", err) } }
func (m *Manager) snapshot() (*stream.Stream, bool, Availability) { m.mu.Lock(); defer m.mu.Unlock(); return m.selected, m.listening, m.availability }
func (m *Manager) setState(state State, message string, playerStatus *player.Status) { m.mu.Lock(); defer m.mu.Unlock(); m.state = state; m.lastError = message; m.updateStatusLocked(playerStatus) }

func (m *Manager) updateStatusLocked(playerStatus *player.Status) {
	status := m.status; status.State = string(m.state)
...
func (m *Manager) notify() { select { case m.wake <- struct{}{}: default: } }
