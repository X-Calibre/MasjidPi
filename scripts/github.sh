...
prepare_release() {
    ...
    if [[ -x "$PROJECT_ROOT/masjidframe" && -f "$local_version_file" ]]; then
        RELEASE_DIR="$PROJECT_ROOT"
        RELEASE_VERSION="$(cat "$local_version_file")"
        info "Using bundled MasjidFrame release $RELEASE_VERSION."
        return
    fi
    ...
    [[ "$latest_tag" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]] \
        || die "Unable to determine a valid latest MasjidFrame release."
    ...
    case "$ARCH" in
        ...
        aarch64)
            ...
            ;;
        armv7l)
            die "No official MasjidFrame release is available for armv7l. Use --source instead."
            ;;
        *)
    ...
    local archive_name="masjidframe-${RELEASE_VERSION}-linux-${RELEASE_ARCH}.tar.gz"
    ...
    local download_dir
    download_dir="$(mktemp -d /tmp/masjidframe-release.XXXXXX)"

    RELEASE_DIR="$download_dir/masjidframe-${RELEASE_VERSION}-linux-${RELEASE_ARCH}"

    info "Downloading MasjidFrame ${RELEASE_VERSION} (${RELEASE_ARCH})..."
    ...
    success "MasjidFrame ${RELEASE_VERSION} downloaded and verified."
}
